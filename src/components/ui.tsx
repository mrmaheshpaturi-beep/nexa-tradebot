...
export function FilterBar({ search, onSearch, children }: { search?: string; onSearch?: (value: string) => void; children?: ReactNode }) {
  return <div className="filter-bar">{onSearch && <label className="search-input"><Search size={15} /><input aria-label="Search" value={search} onChange={(e) => onSearch(e.target.value)} placeholder="Search…" /></label>}{children}<button className="select-btn" type="button">Last updated: now <ChevronDown size={14} /></button></div>
}
