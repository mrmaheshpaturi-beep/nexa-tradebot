...
export interface Order { id: string; ticket: string; symbol: string; direction: Direction; type: 'Market' | 'Buy Limit' | 'Sell Limit' | 'Buy Stop' | 'Sell Stop'; volume: number; entry: number; stopLoss: number; takeProfit: number; status: 'PENDING' | 'FILLED' | 'CANCELLED'; simulated: true }
