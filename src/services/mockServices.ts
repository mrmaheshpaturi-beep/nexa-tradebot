import type { AccountSnapshot, AppNotification, BacktestConfig, BacktestResult, BrokerAccount, Candle, Position, Quote, RiskEvent, RiskProfile, Signal, Strategy, SystemHealth, Trade } from '../domain/types'
...
export interface OrderService { simulateOrder(input: { symbol: string; direction: 'BUY' | 'SELL'; volume: number }): Promise<{ ticket: string; accepted: true; simulated: true }> }
...
export class MockMarketDataService implements MarketDataService {
  ...
  getCandles = async (_symbol: string) => wait(Array.from({ length: 80 }, (_, i) => {
...
export class MockOrderService implements OrderService { simulateOrder = async (_input: { symbol: string; direction: 'BUY' | 'SELL'; volume: number }) => wait({ ticket: `SIM-${Date.now().toString().slice(-6)}`, accepted: true as const, simulated: true as const }) }
