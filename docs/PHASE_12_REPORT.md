...
BACKTEST ≠ DEMO enforced on `BacktestRun` and `ResearchComparison`. Promote API returns 403. LIVE hard-blocked. Audit script `scripts/phase12-analytics-backtest-audit.sh`. Sole `order_send` unchanged at `trading-engine/src/nexa_mt5/execution.py::authorized_order_send`.
...
| Phase 11 regression (sample) | see gates |
| Vitest / tsc / eslint / build / python / audit | see final status |
...
- Real MT5 DEMO outcome volume for analytics depends on Phase 10/11 Windows DEMO usage.
